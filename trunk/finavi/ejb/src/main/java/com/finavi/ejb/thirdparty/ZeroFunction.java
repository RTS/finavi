    ...
    implements Serializable, Cloneable
{
    ...
    public synchronized void setAbsoluteError(double d)
    {
        if(d < 0.0D)
        {
            ...
            throw new IllegalArgumentException("Negative+" +aobj);
        }
        errorAbsolute = d;
    }
    ...
    public synchronized void setRelativeError(double d)
    {
        if(d < 0.0D || d > 1.0D)
        {
            ...
            throw new IllegalArgumentException("NotInInterval"+aobj);
        }
        errorRelative = d;
    }
...
label0:
            for(iterations[j] = 0; iterations[j] < maxIterations; iterations[j]++)
            {
